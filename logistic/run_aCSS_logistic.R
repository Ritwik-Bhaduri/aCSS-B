...
source(paste0("../aCSS code/aCSS_source.R"))
...
start_time <- Sys.time()
pval_matrix <- foreach(k = 1:250, .combine = rbind, .inorder = FALSE) %dopar%{
  cat(sprintf("Running iteration k: %d, seed: %d\n", k, k))
  run_one_trial(M=250, seed= k, example = 'logistic')
}
...
saveRDS(pval_list,file="files/pval_ACSS.rds")
saveRDS(end_time-start_time,file="files/time_ACSS.rds")
